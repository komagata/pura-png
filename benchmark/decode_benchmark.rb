  ...
  def self.run(input)
    unless File.exist?(input)
      ...
      unless generate_test_image(input)
        $stderr.puts "Error: could not generate test image. Provide an existing PNG file."
        exit 1
      end
    end
    ...
    # pura-png
    results << bench("pura-png") do
      image = Pura::Png.decode(input)
    ...
    begin
      require "chunky_png"
      results << bench("chunky_png") do
        img = ChunkyPNG::Image.from_file(input)
        img.width * img.height * 3
      end
    rescue LoadError
      results << { name: "chunky_png", time: nil, output_size: nil, note: "not installed" }
    end
